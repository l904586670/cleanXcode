...
def xls_to_json(xls_file):
  ...
  for i in range(num_rows):
    ...
    replace_value = worksheet.cell_value(i, 3)
    if replace_value != "":
      if replace_value != "??":
        value = replace_value
...
def exportJson(excelPath):
  ...
  outPath = filter_path + "/filter_result.json"
  exportData = xls_to_json(excelPath)
  data_write_to_path(exportData, outPath)
